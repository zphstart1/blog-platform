...
flowchart LR
    ...
    A -->|Tiny| S3_D[直接 S3]
    A -->|Small| S2_S[简化 S2]
    A -->|Medium+| S1[阶段1: 需求评审]
    S1 --> S2[阶段2: 技术方案]
    S2_S --> S3[阶段3: 代码实现]
    S2 --> S3
    S3_D --> S3
    S3 --> S4[阶段4: 测试验证]
    S4 --> |通过| G4[Gate 4: 浏览器验证]
    S4 --> |Bug| S3
    G4 --> S5[阶段5: 部署上线]
    S5 --> G5[Gate 5: 健康检查]
    G5 --> S6[阶段6: 交付产品]
...
```
负责人：团队领导 (team-lead)
规则：见 workflows/complexity-assessment.md
产出：任务等级（Tiny/Small/Medium/Large/XLarge）+ 管道策略

Tiny  → 跳过 S1、S2，直接进入 S3
Small → 跳过 S1，S2 简化版
Medium+ → 完整 6 阶段流水线
```
...
输入：本任务中的所有发现
产出：
  - 新陷阱 → 追加到 agent-team/knowledge/learned-lessons.md
  - 状态变更 → 更新 agent-team/knowledge/context.md
  - 经验确认 → 标注已有经验为"已验证有效"
```
...
> 质量门禁详细定义见 `protocols/quality-gates.md`
